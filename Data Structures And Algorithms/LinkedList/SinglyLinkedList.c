...
void ABG_InsertAtPosition(NODE** ABG_psnHead, int ABG_iNum, unsigned int ABG_iPosition)
{
	...
	else if (ABG_iPosition == ABG_CountNode(*ABG_psnHead) + 1)
	{
		free(ABG_psnNewNode);
	...
	else if (ABG_iPosition > ABG_CountNode(*ABG_psnHead) + 1 || ABG_iPosition <= 0)
	{
		free(ABG_psnNewNode);
		ABG_psnNewNode = NULL;
...
void ABG_DeleteNode(NODE** ABG_psnHead, unsigned int ABG_iPosition)
{
	...
	else if (ABG_iPosition == ABG_CountNode(*ABG_psnHead))
	{
	...
	else if (ABG_iPosition > (ABG_CountNode(*ABG_psnHead)) || ABG_iPosition <= 0)
	{
		printf("\n\n+-+-+-+-+-+-+-+ ENTER VALID POSITION +-+-+-+-+-+\n\n");
