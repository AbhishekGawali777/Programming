...
void Display(void)
{
	...
	for (int i = 0; i < 10; i++)
	{
		...
		glColor3f(gfColors[i].r, gfColors[i].g, gfColors[i].b);

		for (GLfloat fAngle = 0.0f; fAngle <= 2 * PI; fAngle = fAngle + 0.001)
		{
			glVertex3f((GLfloat)cos(fAngle), (GLfloat)sin(fAngle), 0.0f);
		}
